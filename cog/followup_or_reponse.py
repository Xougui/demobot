...
class FollowupOrResponse(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:  # initialisation de la classe ( obliatoire )
        self.bot = bot
    ...
    async def followup_or_response(
        self, interaction: discord.Interaction
    ) -> None:  # discord.Interaction est l'objet qui va contenir les infos de l'interaction
        ...
        await interaction.response.defer(
            ephemeral=True
        )  # ON AS UTIILISER DEFER POUR POUVOIR ENVOYER UNE FOLLOWUP APRES
        ...
        await interaction.followup.send(
            "Ceci est un message de followup !", ephemeral=True
        )
        ...
        await interaction.followup.send(
            "Ceci est un second message de followup !", ephemeral=True
        )
        await interaction.followup.send(
            "Ceci est un troisième message de followup !", ephemeral=True
        )
        # ...


async def setup(bot: commands.Bot) -> None:  # fonction setup obligatoire dans un cog
